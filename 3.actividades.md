...
14. Instala el plugin Gradle para IntelliJ IDEA.
<hr>

15. Crea un proyecto nuevo que utilice el sistema de construcción `Gradle` y  que contenga la clase _`Empleado`_. 
<hr>
...
17. Busca la tarea de Gradle para realizar los tests y ejecútalos. Puedes acceder a ellos en `View` -> `Tools Windows` -> `Gradle`
...
18. En los siguientes ejercicios vamos a trabajar la **cobertura de código** de los tests.
Antes de iniciar, consulta la documentación disponible en https://www.jetbrains.com/help/idea/code-coverage.html
<hr>

19. Clona el repositorio https://github.com/jamj2000/Test-JUnit5-Aritmetica en tu equipo local.
<hr>
...
23. Para generar un informe de la cobertura, ve al menú y luego a `Run` -> `Generate coverage report`. Al pulsar en esta opción del menú se creará una carpeta `htmlReport` y dentro una página `index.html` con el informe. Puedes abrirla con el comando:
    `firefox htmlReport/index.html` 
<hr>
